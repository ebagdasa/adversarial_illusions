...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def encode_audio(self, audio_paths):
        if 'hack' in audio_paths[0]:
            print(f'IMPORTANT!! OVERWRITING AUDIO LOADING WITH MODIFIED INPUT: {audio_paths[0]}')
            inputs = {ModalityType.AUDIO: torch.load(audio_paths[0]).to(self.device)}
        else:
    ...
    def encode_image(self, image_paths):
        ...
        if 'embed' in image_paths[0]:
            print(f'IMPORTANT!! OVERWRITING IMAGE LOADING WITH THE EMBEDDING: {image_paths[0]}')
            image_embeds = torch.load(image_paths[0]).to(self.device)
            inputs_llama = self.llama_proj(image_embeds).unsqueeze(1) # bsz x 1 x llama_size
            atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(self.device) # bsz x 1
            return inputs_llama, atts_llama       
            
        elif 'hack' in image_paths[0]:
            print(f'IMPORTANT!! OVERWRITING IMAGE LOADING WITH MODIFIED INPUT: {image_paths[0]}')
            inputs = {ModalityType.VISION: torch.load(image_paths[0]).to(self.device)}
        else:
    ...
    def prompt_wrap(self, img_embeds, input_ids, target_ids, attention_mask):
        ...
        print(f'images: ', img_embeds.shape)
        print(f'p_before_embeds: ', p_before_embeds.shape)
        print(f'p_after_embeds: ', p_after_embeds.shape)
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        # feature_embeds = self.llama_proj(feature_embeds).unsqueeze(1)
        print('feature_embeds: ', feature_embeds.shape)
